...
> ```
>
> 不要为子模块再抽统一的生命周期基类或共享库：组件的销毁条件与清理时机各不相同
> （如 `WebViewHelper` 的 `isAlive` 还要求 `webView` 非空、`PageTool` 与持有它的 Model 销毁同步），
> 统一基类收不了这些差异，只会多一层间接。每个子模块按上述三件套手写即可，重复的十几行是可接受的。
>
> **4. 网络请求必须包装**
>
