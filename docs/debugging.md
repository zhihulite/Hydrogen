...
`init_app.lua` 给 `print` 打了补丁：先调引擎原 `print`（即上面这条 logcat），再看设置页的
「调试模式」（`SharedDataKeys.DEBUG_MODE`）——开着才额外弹模态对话框（标题 Print）。
所以关掉调试模式只是不弹窗，logcat 照旧。判据是在每次调用时惰性读的，因为 `init_app` 跑在
...
相当于设备端 REPL。外部脚本也可以用 scheme 拉起（域外路径每次执行都会先弹确认框，单次有效）：
...
`components/views/web_view_helper.lua` 无条件开了 `webContentsDebuggingEnabled = true`，不看构建
