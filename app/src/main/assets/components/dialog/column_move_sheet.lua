-- components/dialog/column_move_sheet.lua
...
function M.show(options)
  ...
  local root = loadlayout(layout, sheetViews)
  tipView = sheetViews.tip
  local recyclerView = sheetViews.recycler

  -- 关闭按钮
  sheetViews.close_btn.onClick = function()
    bottomSheet.dismiss()
  end

  -- 新建专栏
  sheetViews.new_btn.onClick = function()
  ...
  local function loadColumns()
    local url = nextUrl or "https://api.zhihu.com/members/" .. Extensions.Account.getUserId() .. "/owned-columns?type=" .. contentType .. "&id=" .. contentId
    ...
    NetWork.get(url, headers, function(code, content)
      ...
      if not ok or not result or not result.data then
        tip("专栏列表加载失败")
        if options.onError then options.onError("专栏列表解析失败") end
        return
      end
      ...
      -- 仅在拿到与当前页不同的下一页地址时继续翻页，避免地址缺失时回退首页无限重复请求
      local nextPage = result.paging and not result.paging.is_end and result.paging.next
      if nextPage and nextPage ~= url then
        nextUrl = nextPage
        loadColumns()
      end
    end)
  ...
  -- 确认按钮
  sheetViews.confirm_btn.onClick = function()
