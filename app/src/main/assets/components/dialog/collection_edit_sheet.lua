-- components/dialog/collection_edit_sheet.lua
-- 创建/编辑收藏夹表单弹窗
...
function M.show(opts)
  ...
  local layout = {
    ...
    layout_height = "wrap_content",
    {
      ...
      L.edit("name_input", AppTextStyle.bodyMedium, "请输入收藏夹名称", {
        layout_height = "wrap_content",
        text = opts.name or "",
        layout_marginBottom = "16dp",
      }),
      ...
      L.edit("desc_input", AppTextStyle.bodyMedium, "请输入描述", {
        layout_height = "wrap_content",
  ...
  self.views.save_btn.onClick = function()
    ...
    -- 开关文案是「仅自己可见」，勾选时提交 is_public = false
    local privateSwitch = self.views.public_switch
    local isPublic = true
    if privateSwitch then isPublic = not privateSwitch.isChecked() end
    local defaultSwitch = self.views.default_switch
    local isDefault = false
    if defaultSwitch then isDefault = defaultSwitch.isChecked() end
    ...
    local function doRequest(url, postData)
      if isEdit then
        NetWork.put(url, postData, Headers.post, function(code)
          self.isSaving = false
