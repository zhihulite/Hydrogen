-- components/dialog/collection_move_sheet.lua
...
-- form 字符串体引擎不会自动补 content-type，需要显式带上；
-- table.merge 返回新表，Headers.defaultHead 每次读取都是现构建的头表，不受影响
local function formPutHead()
  return table.merge(Headers.defaultHead, {
...
--- @param opts.autoToggle boolean 是否自动切换默认收藏夹（true: 自动取反第一个收藏夹，不显示面板；false: 显示面板让用户选择）
...
function M:autoToggleDefault(opts)
  ...
  NetWork.get(url, Headers.defaultHead, function(code, content)
    ...
    NetWork.put(putUrl, putData, formPutHead(), function(code)
      if code == 200 then
...
--- 显示收藏夹选择面板
function M:showSelectionSheet(opts)
  local self = {}
  ...
  -- 初始化 RecyclerView
  local lm = SafeLinearLayoutManager(activity)
  ...
  -- 滚到倒数第 3 项时取 nextUrl 指向的下一页
  self.views.recycler_view.addOnScrollListener(luajava.override(RecyclerView.OnScrollListener, {
    onScrolled = function(super, rv, dx, dy)
      if dy > 0 and not self.isLoading and self.nextUrl then
        if rv.layoutManager.findLastVisibleItemPosition() >= self.adapter.itemCount - 3 then
...
function M:loadMoreCollections()
  ...
  NetWork.get(url, Headers.defaultHead, function(code, content)
    ...
    -- 列表结束后 next 仍会返回地址，只在未结束且地址变化时留作下一页
    local nextPage = data.paging and not data.paging.is_end and data.paging.next
    if type(nextPage) == "string" and nextPage ~= "" and nextPage ~= url then
