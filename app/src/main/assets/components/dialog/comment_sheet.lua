-- components/dialog/comment_sheet.lua
-- 评论sheet
...
local CommentModel = require("models.content.comment_model")
...
function M:initListView()
  ...
  model:setupSingle(views.recycler_view)
  model:ensureLoaded()
  ...
  model:addListener("moreCommentsClick", function(parentId)
    M.show({
      contentId = parentId,
      contentType = "comment",
...
function M:openCommentEditor()
  if not Extensions.Account.isLoggedIn() then
    tip("请登录后使用")
    Router.go("login")
    return
  end

  return self:showReplyEditor(nil, nil, "发布失败")
end
...
function M:onCommentClick(item)
  -- item.title 是带「作者」标签与被回复人的展示串，编辑器 placeholder 只要纯昵称
  self:showReplyEditor(item.id, item.author and item.author.name or item.title)
end

--- 打开评论编辑器
--- @param commentId string|nil 被回复的评论 ID，nil 表示发表新评论
--- @param authorName string|nil 被回复人昵称，用于编辑器 placeholder
--- @param errorTip string|nil 失败提示兜底文案
function M:showReplyEditor(commentId, authorName, errorTip)
  if not Extensions.Account.isLoggedIn() then
    tip("请登录后使用")
    return
  end

  local CommentEditorSheet = require("components.dialog.comment_editor_sheet")
  ...
  CommentEditorSheet.show({
    ...
    onSuccess = function()
      -- 面板可能已被 close 销毁，此处回调不经 Model 的 runIfAlive
      if self.model then self.model:refresh() end
      if commentId then tip("回复成功") end
    end,
    onError = function(err)
      tip(err or errorTip or "回复失败")
    end
  })
end
...
function M:handleDislike(item)
  if not Extensions.Account.isLoggedIn() then tip("请登录后使用") return end
...
function M.show(options)
  ...
  self.currentOrder = self.model.orderBy
  ...
  -- 面板关闭时销毁 Model：destroy 沿类链上溯销毁 pageTool，飞行中的网络回调
  -- 才会被 runIfAlive 拦下，同时清空监听器与 Java 视图引用
  self.bottomSheet.onDismiss = function()
    self:close()
  end
