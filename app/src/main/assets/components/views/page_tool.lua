-- components/views/page_tool.lua
...
-- 读取屏蔽词配置并按空白切分，未配置时返回 nil
local function getBlockWords()
  local raw = Extensions.Config.getString(Constants.SharedDataKeys.BLOCK_WORDS)
...
-- words 由调用方在每页解析前用 getBlockWords 取一次后传入
local function hasBlockWord(text, words)
  if not text or not words then return false end
...
local function setupRecyclerView(rv, adapter, onLoadMore)
  if not rv.layoutManager then
    rv.layoutManager = SafeLinearLayoutManager(activity, RecyclerView.VERTICAL, false)
  end
  rv.adapter = adapter
...
-- rv/sr 的 Java 引用可能已被 luajava.clear 置空，动画与 refreshing 复位前需确认引用有效
local function finishRefresh(rv, sr, withAnimation)
  local function resetRefreshing()
    if Helpers.UI.isViewAlive(sr) then
      sr.refreshing = false
    end
  end

  if withAnimation and Helpers.UI.isViewAlive(rv) then
...
-- 未登录守卫：未登录时提示并返回 false，调用方据此中断加载
local function requireLogin()
  if Extensions.Account.isLoggedIn() then
    return true
  end
  tip("请登录后使用")
  return false
...
function M:setupLoadFunction()
  ...
  self.loadFunction = function(key, loadPrev, isRefresh)
    ...
    if self.needLogin and not requireLogin() then
      return
    end
    ...
    -- 请求前置位加载态：回调可能被同步执行（如签名失败直接回调 401），
    -- 回调里对 canLoad / refreshing 的恢复必须是最终值
    ...
    NetWork.get(url, headers, self:runIfAlive(function(code, content)
      ...
      state.loadPrev = false
      -- 接口可能 200 返回非 JSON（HTML 错误页）：裸 decode 抛错会把
      -- finishRefresh 一起跳过，刷新转圈永不复位
      ...
      local function update()
        ...
        if state.needCheckFullPage and Helpers.UI.isViewAlive(self.contentView) then
          self.contentView.postDelayed({
            run = self:runIfAlive(function()
              if not Helpers.UI.isViewAlive(rv) then return end
        ...
        local blockWords = getBlockWords()
        for _, item in ipairs(items) do
          ...
          if not hasBlockWord(contentText, blockWords) then
            parser(item, page.data)
          end
        end

        -- onLoad 在本页解析完成后调用，回调内可见已写入 page.data 的条目；
        -- oldCount 取自解析之前，onLoad 自己插入的条目也落在插入通知区间内
        if self.onLoad then
          self.onLoad(data, page.data, key, isFirst)
        end
      ...
      if isRefresh and Helpers.UI.isViewAlive(rv) then
        local animate = rv.animate().alpha(0)
        animate.duration = 100
        animate.withEndAction({
          run = self:runIfAlive(update)
        }).start()
       else
        update()
      end
...
function M:setOnTabListener(callback)
  ...
  self.tabLayout.addOnTabSelectedListener(luajava.createProxy(TabLayout.OnTabSelectedListener, {
