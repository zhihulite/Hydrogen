-- components/views/web_view_helper.lua
-- webview 助手
...
local LuaWebView = luajava.bindClass("org.luajvm.android.widget.LuaWebView")
...
local function getMimeType(acceptTypes)
  ...
  -- 未写 accept 的 <input type="file">，getAcceptTypes() 返回长度 1、元素为空串的数组
...
function M:enableFileUpload(owner)
  ...
  luajava.createProxy(ActivityResultCallback, {
    onActivityResult = function(uri)
      local callback = self.filePathCallback
      self.filePathCallback = nil
      if not callback then return end
      -- 用户取消选择时 uri 为空，必须回传 nil 结束本次选择，否则 WebView 的
      -- <input type="file"> 会一直卡在等待状态
      if not uri then
        callback.onReceiveValue(nil)
        return
      end
      local uriArray = luajava.newArray(Uri, 1)
      uriArray[0] = uri
...
function M:initFindListener()
  ...
  self.webView.setFindListener(luajava.createProxy(WebView.FindListener, {
...
-- fontScale 是 Java float，×100 后带浮点尾数，四舍五入取整才能喂给 setTextZoom(int)
local textZoom = math.floor(fontScale * 100 + 0.5)
...
function M:initDownloadListener()
  if not self:isAlive() then return self end
  self.webView.setDownloadListener(luajava.createProxy(DownloadListener, {
...
function M:setWebViewClient(callbacks)
  ...
  local defaultCallbacks = {
    ...
    onReceivedSslError = function(view, handler, error)
      -- 证书校验失败即中断加载，防止中间人用自签证书截获登录 Cookie
      handler.cancel()
    end,
  ...
  self.webView.setWebViewClient(luajava.createProxy(
  LuaWebView.Creator, merged))
  return self
end
...
function M:setWebChromeClient(callbacks)
  ...
  local rootView = activity.window.decorView

  local defaultCallbacks = {
    onShowCustomView = function(view, callback)
      _G.webViewfullscreenMode = true
      -- 全屏视图挂在 decorView 上，存到 self 才能在 destroy 时摘除
      self.webVideoView = view
      self.webVideoRoot = rootView
      self.savedScrollY = self:getScrollY()
    ...
    onHideCustomView = function()
      ...
      if self.webVideoView then
        rootView.removeView(self.webVideoView)
        self.webVideoView = nil
        self.webVideoRoot = nil
      end
      Helpers.UI.runDelayed(200, self:runIfAlive(function()
        self.webView.scrollTo(0, self.savedScrollY or 0)
      end))
    end,
  ...
  for key, defaultFunc in pairs(defaultCallbacks) do
    merged[key] = self:runIfAlive(defaultFunc)
  end
  ...
  self.webView.setWebChromeClient(luajava.createProxy(
  LuaWebView.ChromeCreator, merged))
  return self
end
...
function M:destroy()
  -- 全屏播放中销毁：把挂到 decorView 上的视频视图摘掉
  if self.webVideoView then
    if self.webVideoRoot then
      self.webVideoRoot.removeView(self.webVideoView)
    end
    self.webVideoView = nil
    self.webVideoRoot = nil
    _G.webViewfullscreenMode = false
