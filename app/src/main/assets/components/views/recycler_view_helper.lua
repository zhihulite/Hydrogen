-- components/views/recycler_view_helper.lua
...
function M:removeHeader(index)
  ...
  if self.concatAdapter and adapter then
    self.concatAdapter.removeAdapter(adapter)
  end
  return self
end
...
function M:getAdapter()
  ...
  -- 多参数直传：ConcatAdapter(a, b, c)，元素与 Adapter... 形参逐个匹配；
  --   避免依赖 table 传 varargs 的隐式展开语义
  self.concatAdapter = ConcatAdapter(table.unpack(adapters))
