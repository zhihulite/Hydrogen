-- components/adapter/simple_recycler_adapter.lua
-- 简洁的 RecyclerView 适配器

local M = {}

import "org.luajvm.android.widget.LuaCustRecyclerAdapter"

--- 创建适配器
--- @param config.items table 数据列表
--- @param config.onCreateView function(viewType) 返回 view, views
--- @param config.onBind function(views, item, position, holder, adapter) 绑定数据
--- @param config.getItemViewType function(position, item)|nil 视图类型分派
--- @return LuaCustRecyclerAdapter
function M.new(config)
  local items = config.items or {}
  local onBind = config.onBind
  local getItemViewType = config.getItemViewType or function() return 0 end
  local onCreateView = config.onCreateView

  if not onCreateView then
    error("必须提供 onCreateView 参数")
  end

  if not onBind then
    error("必须提供 onBind 参数")
  end

  -- 先声明再赋值：onBind 需要把适配器本身回传给调用方，用于按位置刷新
  local adapter
  adapter = LuaCustRecyclerAdapter(activity, LuaCustRecyclerAdapter.Creator({
    getItemCount = function()
      return #items
    end,

    getItemViewType = function(position)
      return getItemViewType(position, items[position + 1])
    end,

    onCreateViewHolder = function(parent, viewType)
      local view, views = onCreateView(viewType)
      local holder = LuaCustRecyclerAdapter.LuaCustRecyclerHolder(view)
      holder.views = views or {}
      return holder
    end,

    onBindViewHolder = function(holder, position)
      local item = items[position + 1]
      onBind(holder.views, item, position, holder, adapter)
    end,

    onViewRecycled = function(holder) end,
  }))

  return adapter
end

-- 辅助方法：从 layout 模板创建 view 和 views
function M.inflate(layout)
  local views = {}
  local view = loadlayout(layout, views)
  return view, views
end

return M