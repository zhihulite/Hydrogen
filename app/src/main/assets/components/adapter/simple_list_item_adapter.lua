-- components/adapter/simple_list_item_adapter.lua
-- 基于 LuaListItemAdapter 的 Material 3 列表适配器
-- 自动处理分段圆角，分组逻辑由外部传入

import "org.luajvm.android.widget.LuaListItemAdapter"
...
function M.new(config)
  ...
  local getItemViewType = config.getItemViewType or function(position, item) return 0 end
  ...
  -- 先声明再建 creator：onBind 需要把适配器本身回传给调用方
  local adapter
  local creator = luajava.createProxy(LuaListItemAdapter.Creator, {    getItemCount = function()
      return #items
    end,
    ...
    onBindViewHolder = function(holder, position)
      ...
      onBind(views, item, position, posInSection, sectionSize, adapter)
    end,

    onViewRecycled = function(holder) end,
  })

  -- 闭包建成后赋值，onBind 经第六参把适配器回传给调用方
  adapter = LuaListItemAdapter(activity, creator)
  return adapter
end
