...
local _class_default_args = setmetatable({}, { __mode = "k" })
local _final_class = {}

---获取继承链上的方法列表（rawget 只取每一级自己定义的方法，不做继承解析）
---@param cls table 类
...
local function get_chain_methods(cls, method, direction)
  ...
  if direction == "down" then
    ...
    while cur do
      ...
      local fn = vtb and rawget(vtb, method)
      if fn then
...
local function ext_new(self, ...)
  ...
  -- 默认参数：仅无参时生效，原样传递给所有 ctor；按类自身声明取值，不沿继承链解析
  local def = _class_default_args[self]
  if #args == 0 and def then
  ...
  setmetatable(obj, {
    __index = function(t, k)
      ...
      if dir then
        ...
        return function(_, ...)
          -- 首个形参吃掉冒号调用传入的接收者，self 一律使用捕获的 t
          local ret
  ...
  -- 抽象方法检查（收集所有祖先类声明的抽象方法，在构造副作用之前拦截）
  ...
  for name, declarer in pairs(all_abstract) do
    -- 实现必须由 self 到声明类之间的某一级自己定义（rawget），声明类自身的同名函数不算实现
    local implemented = false
    local cur = self
    while cur and cur ~= declarer do
      local cur_vtb = _class_vtb[cur]
      if cur_vtb and rawget(cur_vtb, name) ~= nil then
        implemented = true
        break
      end
      cur = cur.super
...
local function class(super, default_args)
  ...
  if super then
    ...
    -- 子类 vtb 未定义的键逐级向上解析，不写回子类，rawget 始终只反映本类定义
    setmetatable(vtb, {
      __index = function(t, k)
        return super_vtb[k]
      end,
    ...
    cls.super = super
    _class_default_args[cls] = default_args
