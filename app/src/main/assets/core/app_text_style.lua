...
-- 布局尺寸配置（页面布局设置页写入，单位 dp，默认值见 constants.lua 的 SETTINGS）
local LayoutKeys = Constants.SharedDataKeys
local pageMargin = Extensions.Config.getNumber(LayoutKeys.PAGE_MARGIN)
local cardGap = Extensions.Config.getNumber(LayoutKeys.CARD_GAP)
local cardPadding = Extensions.Config.getNumber(LayoutKeys.CARD_PADDING)

-- 卡片样式：运行时 dp2px 计算，脚本可随时覆写（如紧凑模式全局调边距）
_G.AppCardStyle = {
  -- 基础卡片样式
  basic = {
    marginLeft = dp2px(pageMargin),
    marginRight = dp2px(pageMargin),
    marginTop = dp2px(cardGap),
    marginBottom = dp2px(0),

    innerPaddingLeft = dp2px(cardPadding),
    innerPaddingRight = dp2px(cardPadding),
    innerPaddingTop = dp2px(cardPadding),
    innerPaddingBottom = dp2px(cardPadding),
  },
  -- 子卡片样式（嵌套卡片）：间距不随卡片间距归零，嵌套层始终留出分隔
  child = {
    marginLeft = dp2px(8),
    marginRight = dp2px(8),
    marginTop = dp2px(math.max(4, cardGap)),
    marginBottom = dp2px(0),
  },
  -- 设置/关于项目卡片样式：内边距固定，列表行保持紧凑
  setting = {
    marginLeft = dp2px(pageMargin),
    marginRight = dp2px(pageMargin),
    marginTop = dp2px(4),
...
_G.AppSpacing = {
  content = dp2px(pageMargin + 4), -- 弹窗/表单/页面头部的内容边距
