...
--- 检查更新
--- @param callback function|nil 回调 (hasNew, msg, result)
--- @param silent boolean|nil true 时只回调、不弹提示
function M.checkUpdate(callback, silent)
  if isChecking then
    tip("正在检测中")
    return
  end
  ...
  -- 更新接口在 gitee 域名，显式传一张独立请求头；_G.Headers.defaultHead 带的是
  -- zhihu.com 的登录 cookie，不能随更新请求外发
  local started = NetWork.get(update_api, { ["User-Agent"] = "Hydrogen" }, function(code, content)
    isChecking = false
    if code ~= 200 then
      if not silent then
        tip("检测更新失败，请检查网络连接")
    ...
    local remoteVersion = tonumber(updateVersion)
    local hasNewVersion = remoteVersion ~= nil and remoteVersion > currentVersion
    if callback then
  ...
  -- canLoad 被置 false 时 NetWork.get 同步返回 false 且不触发回调，此处复位检测标记
...
function M.showUpdateDialog(force)
  M.checkUpdate(function(hasNew, msg, result)
    -- 检测是异步的，宿主 Activity 可能已结束，此时不能再弹窗
