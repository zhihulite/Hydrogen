...
-- 算法为 32 位字运算（类 SM4），服务端按 32 位语义校验签名。JS 的位运算
-- 本身就是 32 位，JS 代码无需任何截断；Lua 5.4+ 整数为 64 位、位运算在
-- 64 位上进行，故用 toInt32 显式截断，复刻 32 位回绕与移位溢出丢弃
...
local function bytesToInt32(tbl)
  return toInt32((((tbl[1] * 256 + tbl[2]) * 256) + tbl[3]) * 256 + tbl[4])
end
...
function XZSE96V3:rotateXor(x, rot)
  rot = rot % 32
  local u = x & 0xFFFFFFFF
  return toInt32(((u << rot) | (u >> ((32 - rot) % 32))) & 0xFFFFFFFF)
end
...
function XZSE96V3:transformValue(e)
  ...
  return toInt32(r ~ self:rotateXor(r, 2) ~ self:rotateXor(r, 10) ~ self:rotateXor(r, 18) ~ self:rotateXor(r, 24))
end

function XZSE96V3:transformBlock(data)
  local function bytesToInt32Range(tbl, start)
    return toInt32((((tbl[start] * 256 + tbl[start + 1]) * 256) + tbl[start + 2]) * 256 + tbl[start + 3])
  end
  ...
  for r = 1, 32 do
    ...
    local temp = words[r + 1] ~ words[r + 2] ~ words[r + 3] ~ zkVal
    local transformed = self:transformValue(temp)
    words[r + 4] = words[r] ~ transformed
  end
...
function XZSE96V3:reverseTransformBlock(data)
  ...
  for i = 1, 4 do
    ...
    table.insert(unpacked, toInt32(word))
  end
  ...
  for r = 32, 1, -1 do
    local zkVal = self.mapping.zk[r]
    local temp = words[r + 1] ~ words[r + 2] ~ words[r + 3] ~ zkVal
    words[r] = self:transformValue(temp) ~ words[r + 4]
  end
...
function XZSE96V3:processBlocks(data, iv)
  ...
  for _, chunk in ipairs(chunks) do
    local xored = {}
    for i = 1, 16 do
      xored[i] = (chunk[i] or 0) ~ currentChain[i]
    end
...
function XZSE96V3:reverseProcessBlocks(data, iv)
  ...
  for _, chunk in ipairs(chunks) do
    ...
    local plainBlock = {}
    for i = 1, 16 do
      plainBlock[i] = decryptedBlock[i] ~ prevChain[i]
    end
...
function XZSE96V3:b64encode(md5Bytes, device, seed)
  device = device or 0
  -- seed 是明文头首字节，参与 iv 推导，服务端解密 iv 即可还原，本身不参与校验；
  -- 每次取 0..126 随机值，使同一 URL 各次请求的签名互不相同
  seed = seed or math.random(0, 126)
  ...
  for i = 1, 16 do
    transformedHeader[i] = headerBlock[i] ~ self.keyPad[i] ~ 42
  end
  ...
  for i = #combined, 3, -3 do
    local b0 = combined[i] ~ self:rightShift(58, 8 * (shiftCounter % 4))
    shiftCounter = shiftCounter + 1
    local b1 = combined[i - 1] ~ self:rightShift(58, 8 * (shiftCounter % 4))
    shiftCounter = shiftCounter + 1
    local b2 = combined[i - 2] ~ self:rightShift(58, 8 * (shiftCounter % 4))
    shiftCounter = shiftCounter + 1

    local num = b0 + (b1 << 8) + (b2 << 16)
    local c1 = self.base64Chars:sub((num & 63) + 1, (num & 63) + 1)
    local c2 = self.base64Chars:sub(((num >> 6) & 63) + 1, ((num >> 6) & 63) + 1)
    local c3 = self.base64Chars:sub(((num >> 12) & 63) + 1, ((num >> 12) & 63) + 1)
    local c4 = self.base64Chars:sub(((num >> 18) & 63) + 1, ((num >> 18) & 63) + 1)
...
local function getMd5(url)
  ...
  return targetUrl, md5Str, cookie
end

function M.encrypt(url)
  local newUrl, md5Str, cookie = getMd5(url)
