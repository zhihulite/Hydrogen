-- models/user/people_more_model.lua
...
function PeopleMoreModel:parseVideoItem(target)
  if self.moreType:find("详情") then
    ...
    return {
      ...
      title = Helpers.ZhihuItem.titleOf(target),
      preview = target.description or "",
      bottomText = string.format("%d个视频 · %d个赞同", target.zvideo_count or 0, Helpers.ZhihuItem.voteupOf(target)),
    }
  end
end
...
function PeopleMoreModel:parseColumnItem(target)
  return {
    ...
    title = Helpers.ZhihuItem.titleOf(target),
    preview = target.description or "",
    bottomText = string.format("%d篇内容 · %d个赞同", target.items_count or 0, Helpers.ZhihuItem.voteupOf(target)),
  }
end
...
function PeopleMoreModel:createAdapter(dataList)
  local isUnderline = self.moreType:find("划线") ~= nil
  return SimpleRecyclerAdapter.new({
    items = dataList,
    getItemViewType = function(position, item)
      if isUnderline then return 1 end
      return 0
    end,
