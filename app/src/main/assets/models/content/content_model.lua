-- models/content/content_model.lua
...
function ContentModel:ctor(contentId, contentType)
  ...
  self.typeInfo = contentTypes[self.contentType] or contentTypes["article"]
...
function ContentModel:parseResponse(response)
  ...
  if self.contentType == "pin" then
    ...
    data.voteupCount = Helpers.ZhihuItem.voteupOf(response)
  end
  ...
  data.author = Helpers.ZhihuItem.authorOf(response.author)
