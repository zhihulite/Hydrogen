-- models/content/answer_model.lua
...
-- 加载问题信息
function AnswerModel:loadQuestionInfo(callback)
...
function AnswerModel:loadAnswer(answerId, callback, silent)
  ...
  self:fetch(url, nil, function(success, response)
    ...
    local data = {
      ...
      voteupCount = Helpers.ZhihuItem.voteupOf(response),
      ...
      attachmentUrl = (function()
        ...
        local stream = playlist and (playlist.sd or playlist.ld or playlist.hd or playlist.fhd)
        return stream and stream.url or nil
      end)(),
    }
...
function AnswerModel:getAdjacentId(isPrev, fromId)
...
function AnswerModel:getPrevAnswerId(fromId)
  return self:getAdjacentId(true, fromId)
end

function AnswerModel:getNextAnswerId(fromId)
  return self:getAdjacentId(false, fromId)
end
...
function AnswerModel:like(answerId, isCurrentlyLiked, callback)
  if not self:requireLogin(callback) then
    return
  end
  ...
  self:post(url, '{"type":"' .. typeStr .. '"}', { requestHeadKey = Constants.RequestHeadKeys.POST } , function(success)
...
function AnswerModel:thank(answerId, isCurrentlyThanked, callback)
  ...
  if isThank then
    ...
    self:post(url, data, { requestHeadKey = Constants.RequestHeadKeys.POST } , function(success)
    ...
    self:delete(deleteUrl, { requestHeadKey = Constants.RequestHeadKeys.POST } , function(success)
      if callback then callback(success, isThank) end
    end)
  end
end
