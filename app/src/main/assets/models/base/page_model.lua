...
-- 子类可选实现：判断本次响应后是否还有下一页
-- 纯判定，不得推进分页游标或产生其它副作用
-- @param data table 本次 parseResponse 得到的数据列表
-- @param params table 请求参数
-- @return boolean 是否还有下一页
function PageModel:hasMore(data, params)
  return true
end
...
--- 初始化单页模式：绑定 RecyclerView、适配器、下拉刷新与滚动加载更多
--- @param recyclerView RecyclerView 实例
--- @param swipeRefresh SwipeRefreshLayout|nil 下拉刷新组件
function PageModel:setupSingle(recyclerView, swipeRefresh)
  self.recyclerView = recyclerView
  self.swipeRefresh = swipeRefresh

  self.recyclerView.layoutManager = SafeLinearLayoutManager(activity)
...
function PageModel:load(params)
  ...
  self:fetch(url, params, function(success, data)
    ...
    if success and data then
      ...
      self.hasNextPage = self:hasMore(data, params)
