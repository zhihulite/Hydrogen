-- models/base/page_tool_model.lua
-- 分页列表模型 - 负责配置 PageTool，支持单页和多页（带 Tab）

local BaseModel = require("models.base.base_model")
local PageTool = require("components.views.page_tool")
...
function PageToolModel:ctor()
  ...
  self.requestHeadKey = Constants.RequestHeadKeys.DEFAULT_HEAD -- 请求头 key
...
function PageToolModel:getRequestHeaders(key)
  if self.requestHeadKey then
    if not Constants.ValidRequestHeadKeys[self.requestHeadKey] then
...
--- 获取当前页面的 RecyclerView 实例（单页模式直接返回，多页模式返回当前 Tab 的 RecyclerView，前置页面返回 nil）
--- @return RecyclerView|nil
function PageToolModel:getCurrentRecyclerView()
  if not self:isAlive() then return nil end
  if not self.pageTool then return nil end
  if self:isCurrentPagePrePage() then return nil end
