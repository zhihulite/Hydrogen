...
function BaseModel:ctor()
  ...
  self.requestHeadKey = Constants.RequestHeadKeys.DEFAULT_HEAD -- 请求头 key (对应 _G.Headers)
  self.urlProcessor = nil -- URL/Headers 预处理函数 function(url, headers) return newUrl, newHeaders end
  self.isDestroyed = false -- 销毁标志，销毁后回调不再执行
end
...
--- 未登录守卫：未登录时提示并让回调收到失败结果
--- @param callback function|nil 业务回调 function(success)
--- @return boolean 已登录返回 true，未登录返回 false
function BaseModel:requireLogin(callback)
...
function BaseModel:getHeaders(params)
  ...
  if requestHeadKey and not Constants.ValidRequestHeadKeys[requestHeadKey] then
    print(string.format("requestHeadKey 非法: %q", requestHeadKey))
  end
...
--- 请求前置：存活与登录校验、置加载态、生成请求头并预处理 URL
--- @param url string 请求地址
--- @param params table 请求参数（传递给 getHeaders）
--- @param callback function 回调函数 function(success, data, code)
--- @return string|nil url 预处理后的地址，请求被拦下时为 nil
--- @return table|nil headers 预处理后的请求头
function BaseModel:prepareRequest(url, params, callback)
  if not self:isAlive() then return nil end
  if self.needLogin and not Extensions.Account.isLoggedIn() then
...
--- 包装 POST/PUT/DELETE 的网络回调：复位加载态、检查响应码、尽量解码 JSON
--- @param callback function 回调 function(success, data, code)
--- @return function 包装后的网络回调 function(code, content)
function BaseModel:wrapMutationCallback(callback)
  return self:runIfAlive(function(code, content)
...
function BaseModel:fetch(url, params, callback)
  ...
  url = url or self:getUrl(params)
  if not url then
    self:setError("缺少请求地址")
    if callback then callback(false, nil, -1) end
    return
  end

  local headers
  url, headers = self:prepareRequest(url, params, callback)
...
function BaseModel:post(url, postData, params, callback)
  ...
  NetWork.post(url, postData, headers, self:wrapMutationCallback(callback))
end
...
function BaseModel:put(url, putData, params, callback)
  ...
  NetWork.put(url, putData, headers, self:wrapMutationCallback(callback))
end
...
function BaseModel:delete(url, params, callback)
  ...
  NetWork.delete(url, headers, self:wrapMutationCallback(callback))
end
