-- models/feed/follow_content_model.lua
...
function FollowContentModel:createAdapter(dataList)
  return SimpleRecyclerAdapter.new({
    ...
    onBind = function(views, item, position, holder)
      -- holder 复用，无头像时要显式收起，否则残留上一条的图
      if item.avatarUrl and item.avatarUrl ~= "" then
        views.avatar.visibility = View.VISIBLE
        Helpers.Image.load(views.avatar, item.avatarUrl)
       else
        views.avatar.visibility = View.GONE
      end
      ...
      local hasBottom = item.bottomText and item.bottomText ~= ""
