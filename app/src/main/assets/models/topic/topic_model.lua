...
function TopicModel:updateDetailPage(topicInfo)
  ...
  views.detail_followers.text = Helpers.UI.formatNumber(topicInfo.followersCount)
  views.detail_questions.text = Helpers.UI.formatNumber(topicInfo.questionsCount)
  views.detail_best_answers.text = Helpers.UI.formatNumber(topicInfo.bestAnswersCount)
end
...
function TopicModel:parseItem(rawItem, key)
  ...
  if target.type == "answer" then
   ...
   elseif target.type == "zvideo" then
    id = tostring(target.id)
    if excerpt == "" then
      preview = name ~= "" and (name .. " : [视频]") or "[视频]"
    end
   elseif target.type == "pin" then
    id = tostring(target.id)
    -- 想法正文首段作预览，取不到时保留带作者名前缀的默认预览
  ...
  local voteupCount = Helpers.ZhihuItem.voteupOf(target)
