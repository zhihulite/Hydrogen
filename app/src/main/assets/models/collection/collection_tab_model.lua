-- models/collection/collection_tab_model.lua
...
function CollectionTabModel:ctor(userId)
  ...
  self.needLogin = true
  self.userId = userId or Extensions.Account.getUserId()
...
function CollectionTabModel:parseItem(rawItem)
  ...
  return {
    ...
    type = "collection",
    title = Helpers.ZhihuItem.titleOf(collection),
...
function CollectionTabModel:createAdapter(dataList)
  return SimpleRecyclerAdapter.new({
    ...
    onBind = function(views, item, position, holder)
      ...
      views.follower_count.text = string.format("%d人关注", item.followerCount or 0)

      views.lock_icon.visibility = item.isPublic == false and View.VISIBLE or View.GONE
      views.creator_name.text = (item.creator and item.creator.name) and (item.creator.name .. " 创建") or ""
...
function CollectionTabModel:onFirstLoad(data, dataList, key)
  ...
  if key == "followed" and #dataList == 0 then
    table.insert(dataList, EMPTY_FOLLOWING_DATA)
  end
end

local CollectionRecommendModel = require("models.collection.collection_recommend_model")
