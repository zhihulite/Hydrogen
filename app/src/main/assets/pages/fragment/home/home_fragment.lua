...
function HomeFragment:initHomePage(views)
  ...
  local loggedIn = Extensions.Account.isLoggedIn()
  local startIndex
    ...
    onNavigationItemSelected = function(item)
      for i = 1, #displayTabs do
        if item == bottomNavItems["tab_" .. i] then
          ...
          if tab.needLogin and not Extensions.Account.isLoggedIn() then
            tip("请登录后使用")
            return false
          end
...
function HomeFragment:initPageWithModel(pageType, views)
  ...
  local userId = Extensions.Account.getUserId()
...
function HomeFragment:setupDrawerHeader()
  ...
  headerViews.card.onClick = function()
    if Extensions.Account.isLoggedIn() then
      Router.go("people", { id = Extensions.Account.getUserId() })
     else
      Router.go("login")
    end
...
function HomeFragment:setupDrawerMenu()
  ...
  nav.setNavigationItemSelectedListener({
    onNavigationItemSelected = function(menuItem)
      ...
      if info and info.needLogin and not info.isSpecial and not Extensions.Account.isLoggedIn() then
        tip("请登录后使用")
        return false
      end
...
function HomeFragment:loadUserInfo()
  ...
  local userModel = UserModel(Extensions.Account.getUserId())
