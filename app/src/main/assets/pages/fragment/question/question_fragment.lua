-- pages/fragment/question/question_fragment.lua
-- 问题页面，问题详情头部卡片 + 回答列表
...
local QuestionModel = require("models.content.question_model")
...
function QuestionFragment:onDestroy()
  -- 弹窗可能未走 dismiss 就随页面销毁，这里兜底释放 WebView
  if self.excerptWebHelper then
    self.excerptWebHelper:destroy()
    self.excerptWebHelper = nil
  end
...
function QuestionFragment:initViews()
  ...
  -- 只配置下拉刷新配色，监听与转圈由 PageTool 接管（initAnswerList 里注册）
...
function QuestionFragment:updateTopics(topics)
  if not self.headerViews or not self.headerViews.topics_container or not self.headerViews.topics_scroll then return end

  local scroll = self.headerViews.topics_scroll
  ...
  for i, topic in ipairs(topics) do
    ...
    if i < #topics then
      ...
      spacer.layoutParams = LinearLayoutCompat.LayoutParams(dp2px(8), 0)
...
function QuestionFragment:showExcerptDetail()
  ...
  self.excerptWebHelper = webHelper
  webHelper:initSettings():initNoImageMode():setWebViewClient({
  ...
  BottomDialog.show({
    ...
    positiveText = "关闭",
    cancelable = true,
    -- 关闭按钮、返回键、点弹窗外关闭都会走 dismiss，WebView 统一在这里销毁
    onDismiss = self:runIfAlive(function()
      if webHelper == self.excerptWebHelper then
        self.excerptWebHelper = nil
      end
      webHelper:destroy()
    end)
...
function QuestionFragment:loadQuestionDetail()
  ...
  -- 关注/取关只影响关注数与菜单文案，单独监听避免整卡重绘
  self.model:addListener("followChanged", self:runIfAlive(function(data)
    if not self.headerViews then return end
    self.questionData = data
    self.headerViews.follower_count.text = string.format("%d人关注", data.followerCount or 0)
    self:updateFollowMenuItem()
...
function QuestionFragment:toggleFollow()
  ...
  self.model:follow(function(success)
    ...
    -- 详情未加载时 model 走的是关注分支，此时没有 detailData 可比对
    local detail = self.model:getDetail()
    self.questionData = detail or self.questionData
    tip((detail == nil or detail.isFollowing) and "已关注" or "已取消关注")
    self:updateFollowMenuItem()
  end)
end
