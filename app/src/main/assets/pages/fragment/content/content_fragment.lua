-- pages/fragment/content/content_fragment.lua

local BaseFragment = require("pages.base.base_fragment")
local ContentModel = require("models.content.content_model")
...
function ContentFragment:setupToolbar()
  ...
  if self.model:canFavorite() then
    table.insert(menuItems, 2, {
      id = "favorite",
      title = "加入收藏",
      click = function() self:showCollectionMoveSheet() end
    })
  end
  ...
  self.webViewHelper:setWebViewClient({
    shouldOverrideUrlLoading = function(view, url)
      ...
      if contentId and contentType then
        local ColumnMoveSheet = require("components.dialog.column_move_sheet")
...
function ContentFragment:showCollectionMoveSheet()
  CollectionMoveSheet.show({
