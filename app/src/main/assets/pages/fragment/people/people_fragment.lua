-- pages/fragment/people/people_fragment.lua
-- 用户页面 Fragment
...
local PeopleModel = require("models.user.people_model")
...
function PeopleFragment:onCreate(params)
  ...
  self.peopleModel:addListener("userInfoChanged", function(data)
    self.isFollowing = data and data.isFollowing or false
    self:updateUserInfo(data)
    self:setupActionButtons(data)
    if self.blockMenuItem then
      self.blockMenuItem.title = data and data.isBlocking and "取消拉黑" or "拉黑"
    end
  end)
end
...
function PeopleFragment:collectAllBottomViews()
  return self:collectModelBottomViews(self.peopleModel)
end
...
function PeopleFragment:initViews()
  ...
  self.blockMenuItem = menuIdMap and menuIdMap.block
...
function PeopleFragment:updateUserInfo(data)
  ...
  if views.voteup_count then views.voteup_count.text = Helpers.UI.formatNumber(data.voteupCount) .. " 获赞" end
  if views.fans_count then views.fans_count.text = Helpers.UI.formatNumber(data.followerCount) .. " 粉丝" end
  if views.follow_count then views.follow_count.text = Helpers.UI.formatNumber(data.followingCount) .. " 关注" end
  if views.toolbar then views.toolbar.title = data.name or "用户详情" end
end

function PeopleFragment:setupActionButtons(data)
  local views = self.views
  local currentUserId = Extensions.Account.getUserId()

  -- self.userId 来自路由参数，可能是 url_token；data.id 是接口归一化后的数字 id，
  -- 与登录时存入 USER_ID 的值同源，同为 string
  if data and currentUserId == data.id then
