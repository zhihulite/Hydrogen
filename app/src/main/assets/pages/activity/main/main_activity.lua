-- pages/activity/main/main_activity.lua
-- 主页面
...
function MainActivity:ctor()
  ...
  self.containerSeq = 0 -- 容器使用序号，单调自增
end
...
function MainActivity:setupSmartNoImage()
  if Extensions.Config.getBool(Constants.SharedDataKeys.SMART_NO_IMAGE) then
    _G.onStart = function()
      ...
      -- 弹出前置位：下滑或点外部关闭弹窗时，下次 onStart 也不会重复弹出
      local function remind(message, value, okTip)
        self.noImageReminded = true
        Helpers.BottomDialog.confirm(message, function()
          Extensions.Config.set(Constants.SharedDataKeys.NO_IMAGE, value)
          tip(okTip)
        end)
      end

      if isWifiConn and noImage then
        remind("当前在WiFi下，是否关闭不加载图片？", false, "关闭不加载图片成功")
       elseif isMobileConn and not noImage then
        remind("当前在流量下，是否开启不加载图片？", true, "开启不加载图片成功")
      end
    end
  end
end
...
function MainActivity:onCreate(params)
  params = params or {}
  self.startupIntentDataUrl = params.intentDataUrl
...
-- 更新容器使用记录（序号单调自增，同一秒内的连续跳转也能区分新旧）
function MainActivity:updateContainerTime(container, name)
  self.containerSeq = self.containerSeq + 1
  container.setTag(TAG_LAST_TIME, self.containerSeq)
...
function MainActivity:setupFragmentLoader()
  local function fragmentLoader(data)
    ...
    if Extensions.Config.getBool(Constants.SharedDataKeys.USE_SIMPLE_ANIMATION) == false and sharedView then
      -- 共享元素必须在 commit 之前注册：事务执行时即被快照，事后追加无效
      local sourceName = _transition_name .. generate_id()
      local targetName = _transition_name .. generate_id()
      ViewCompat.setTransitionName(sharedView, sourceName)
      transaction.addSharedElement(sharedView, targetName)
      self:setupSharedElementTransition(fragment, fragmentModule, sharedView, targetName)
     else
      -- 设置默认动画（进入和返回）
...
-- 设置共享元素转场（容器变换 + Z轴）
function MainActivity:setupSharedElementTransition(fragment, fragmentModule, sharedView, targetName)

  -- 覆盖默认动画，设置完整转场
  fragmentModule:setOnViewCreatedCallback(function(container)
    -- 目标视图命名：onViewCreated 时转场尚未执行，此处命名仍会被采集
    ViewCompat.setTransitionName(container, targetName)
...
function MainActivity:initViews()
  ...
  -- Fragment 容器与主页已提交后再处理冷启动深链，避免启动参数定时器早于 build()。
  local intentDataUrl = self.startupIntentDataUrl
  self.startupIntentDataUrl = nil
  if intentDataUrl then
    Helpers.UI.runDelayed(500, self:runIfAlive(function()
      self:openExternalUrl(intentDataUrl)
    end))
  end
...
function MainActivity:updateParallelWorld()
  ...
  decorView.postDelayed(self:runIfAlive(function()
...
-- 打开外部来源的 URL：解析出知乎结构则走应用内跳转；
-- 其余链接交给系统浏览器，不在应用内 WebView 载入第三方页面
function MainActivity:openExternalUrl(url)
  local result = Helpers.ZhihuParser.parse(url)
  if result then
    Helpers.ZhihuParser.goFrom(result)
    return
  end

  -- 仅知乎域名留在应用内浏览器，其余交给系统浏览器
  local scheme, host = url:match("^(https?)://([^/?#]+)")
  if not scheme then return end
  host = host:match("^[^@]+@(.+)") or host -- 去掉 userinfo 部分
  host = host:match("^[^:]+") -- 去掉端口
  if host == "zhihu.com" or host:match("%.zhihu%.com$") then
...
function MainActivity:setupTalkBack()
  ...
  local debouncedUpdate = Helpers.UI.debounce(self:runIfAlive(doUpdateAccessibility), 100)

  self.backStackListener = luajava.createProxy(FragmentManager.OnBackStackChangedListener, {
    onBackStackChanged = function()
      debouncedUpdate()
    end
  })
  activity.supportFragmentManager.addOnBackStackChangedListener(self.backStackListener)
...
function MainActivity:onDestroy()
  ...
  -- 摘除 BackStack 监听
  if self.backStackListener then
    activity.supportFragmentManager.removeOnBackStackChangedListener(self.backStackListener)
    self.backStackListener = nil
  end
