-- pages/activity/image/image_activity.lua
-- 图片浏览器 Activity（全屏沉浸 + CoordinatorLayout 自动适配 + 点击切换底栏）

require("init_app")

import "androidx.viewpager2.widget.ViewPager2"
import "org.luajvm.android.widget.LuaPager2Adapter"
...
function ImageActivity:onCreate(params)
  -- data 缺失时退回空表，让下面的空图判定接手；直接赋 nil 会让 # 取长度先报错
  local imageData = params.data or {}
...
function ImageActivity:loadImage(index)
  ...
  .listener(luajava.createProxy(RequestListener, {
...
function ImageActivity:shareCurrentImage()
  ...
  Services.Download.getFileNameAndType(url, nil, self:runIfAlive(function(fileName, mimeType)
    fileName = "shared_" .. fileName

    if luajava.instanceof(drawable, GifDrawableClass) then
      -- GIF：克隆后取完整字节（readAll 在 duplicate 上读，不动原 buffer 的游标）
      local cloned = drawable.constantState.newDrawable().mutate()
      Helpers.UI.shareBytes(LuaUtil.readAll(cloned.buffer), fileName, mimeType)
     else
      -- 静态图：直接获取 bitmap 分享
...
function ImageActivity:downloadCurrentImage()
  ...
  Services.Download.saveImageFromUrl(url)
end
