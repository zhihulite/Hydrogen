-- pages/base/edge_to_edge_utils.lua
-- EdgeToEdge 工具类
--
-- 在 decorView 注册一次 insets 监听，遍历所有已注册的业务 View，
-- 把 systemBars + displayCutout 的 inset 叠加到各 View 原有的 padding/margin 上。
--
-- 特性：
-- - 支持 RTL；lazy 模式下 decorView 自动处理 left/right
-- - 页面销毁时务必调用 remove 清理
--
-- 注意：lazy 模式下 decorView 会自动加 left/right padding，
--       此时不应再传入 start/end 方向的 View，否则 inset 会重复叠加
...
local function apply(record, insets, startInset, endInset)
  ...
  local dirs = record.dirs

  if dirs.top then top = top + insets.top end
  if dirs.bottom then bottom = bottom + insets.bottom end
  if dirs.start then start = start + startInset end
  if dirs["end"] then ending = ending + endInset end
...
local function setupListener()
  ...
  ViewCompat.setOnApplyWindowInsetsListener(activity.window.decorView, function(view, insets)
    ...
    local val = insets.getInsets(bars | cutout)
...
function M.setup(options)
  options = options or {}
  useLazyMode = options.lazy ~= false

  if useLazyMode and (options.start or options["end"]) then
    print("警告: EdgeToEdge 当前为 lazy 模式，decorView 已自动处理 left/right")
